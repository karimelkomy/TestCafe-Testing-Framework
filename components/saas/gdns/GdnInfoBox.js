...
export default class GdnInfoBox extends GenericElement {
	constructor() {
		super();
	...
	async validateOrderStatus(gdnStatus) {
		await this.validateElementVisibility(sprintf(this.orderStatusText, gdnStatus));
	}
	...
	async validateGdnInfoBox({ productRequest }) {
		const { partialGrn, gdnStatus, workspaceDetails, organizationLocation } = productRequest;
		const locationDetails = organizationLocation || workspaceDetails.location;

		if (!partialGrn) {
			await this.validateOrderStatus(gdnStatus); // TODO: Cover partial Grn status validation
		}
