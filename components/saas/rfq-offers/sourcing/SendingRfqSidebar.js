...
export default class SendingRfqSidebar extends GenericElement {
	constructor() {
		...
		this.creditTermDropdownItem =
			'//span[./label[text()="Payment Term"]]//p-multiselectitem/li[@aria-label="%s"]//div[@class="p-checkbox-box"]';
	}
	...
	async addRemark({ requestDetails }) {
		...
		if (requestInfoRemark) {
			await this.fillRemark(requestInfoRemark);
			await this.validateRemark(requestInfoRemark);
		}
	}
	...
	async validateVendorRfqDetails({ vendorsDetails, productsDetails, requestDetails }) {
		...
		for (let vendorDetails of vendorsDetails) {
			...
			await this.clickPlusMinusButton({ vendorName });
