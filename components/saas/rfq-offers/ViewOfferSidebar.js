...
export default class ViewOfferSidebar extends GenericElement {
	constructor() {
		super();
		...
		this.closeButton = '//penny-offer-action-sidebar//button[@data-test-id="close-button"]';
	}
	...
	async validateRemark({ rfqDetails }) {
		const { remark, offerOfflineRemark } = rfqDetails;
		const remarkValue = offerOfflineRemark || remark;

		await this.validateElementVisibility(sprintf(this.remarkText, remarkValue));
	}
	...
	async validate({ rfqId, productsDetails, requestDetails, rfqDetails, organizationDetails, vendorDetails }) {
