...
export default class NegotiateSidebar extends GenericElement {
	constructor() {
		super();
		...
		this.totalWithTaxesndShippingFeeText =
			'//div[contains(@class, "rfq-product-row")][./div[text()=" TOTAL ORDER VALUE "]]//span[contains(.,"%s")]';
		this.totalWithTaxesText = '//span[@data-test-id="negotiate-total-excl-shipping-price-text" and contains(., "%s")]';
		this.taxesText = '//div[contains(@class, "rfq-product-row")][./div[text()=" TAXES "]]//span[contains(.,"%s")]';
		this.shippingFeeInput = '//div[contains(@class, "rfq-product-row")][./div[text()=" SHIPPING FEE "]]//input';
		this.shippingTermText = '//div[contains(@class, "rfq-product-row")][./div[text()=" SHIPPING TERM "]]//span[text()="%s"]';
		...
		this.unitPriceWithTaxesInput = `${this.productRow}//input`;
		this.qtyText = `${this.productRow}/div[.=" %s "]`;
		this.brandText = `${this.productRow}//span[text()=" %s "]`;
		this.vendorOfferCurrency = '//div[@data-test-id="vendor-offer-currency-text" and contains(., "%s")]';
		this.negotiateButton = '//button[.="NEGOTIATE"]';
	}
	...
	async validateShippingFee({ rfqDetails }) {
		...
		await this.validateInputTextEqual(this.shippingFeeInput, formatedNumber(shippingFee));
	}
	...
	async validatePaymentTerm({ rfqDetails }) {
		...
		await this.validateInputTextEqual(this.paymentTermInput, `${formatedNumber(paymentTerm, 0)} ${paymentTermUnit.toUpperCase()}`);
	}

	async validateETA({ rfqDetails }) {
		const { ETA, ETAUnit } = rfqDetails;

		await this.validateInputTextEqual(this.etaInput, `${formatedNumber(ETA, 0)} ${ETAUnit.toUpperCase()}`);
	}
	...
	async validateUnitPrice({ productName, rfqDetails }) {
		...
		await this.validateInputTextEqual(sprintf(this.unitPriceWithTaxesInput, productName), formatedNumber(price));
	}
	...
	async submit({ rfqId, productsDetails, requestDetails, rfqDetails, vendorDetails, organizationDetails }) {
