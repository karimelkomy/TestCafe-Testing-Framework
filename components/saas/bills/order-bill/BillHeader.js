...
export default class BillHeader extends GenericElement {
	constructor() {
		super();
		this.billHeader = '//h1[@data-test-id="bill-header" and contains(text(), "#%s")]';
	...
	async validateBillId({ billId }) {
		await this.validateElementVisibility(sprintf(this.billHeader, billId));
	}
	...
	async validateBillHeader({ billId, vendorDetails }) {
