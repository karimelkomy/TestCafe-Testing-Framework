...
export default class SendingOrdersSidebar extends ProductAndServiceTable {
	...
	async validateCatalogPrice({ requestDetails, rfqDetails, productDetails, organizationDetails }) {
		const { productName, basePrice, contractPrice } = productDetails;
		const { qty } = requestDetails;
		const unitPrice = contractPrice ? contractPrice + this.productPrice.getTaxAmount({ subTotal: contractPrice, rfqDetails }) : basePrice;
	...
	async submit({ vendorsDetails, requestDetails, rfqDetails, productsDetails, organizationDetails }) {
		...
		for (const productDetails of productsDetails) {
			...
			await this.validateCatalogPrice({ requestDetails, rfqDetails, productDetails, organizationDetails });
