...
export default class BuyerRfqPage {
	...
	async submitRfq({ productRequest }) {
		...
		} else if (eSourceSourcing === EsourceSourcing.SOURCE_FROM_PREFERRED_VENDOR) {
	...
	async approveByClientOffer({ productRequest, approverUserDetails }) {
		await this.rfqsSection.approveOfferFromOfferSummary({ productRequest, approverUserDetails });

		await this.offerSuccessSidebar.goToClientRfqList();
	}

	async sendOfferForRevisionByClient({ productRequest }) {
		await this.rfqsSection.sendOfferForRevisionFromOfferSummary({ productRequest });
	}
	...
	async sendOfferForApproval({ productRequest }) {
		await this.rfqsSection.sendOfferForApprovalFromComparisonTable({ productRequest });
	...
	async approveOffer({ productRequest, approverUserDetails }) {
		await this.rfqsSection.submitApproveOffer({ productRequest, approverUserDetails });
