...
export default class GrnPage {
	...
	async submitGrn({ productRequest }) {
		...
		await this.grnProductAndServiceTable.submit({ deliveredQty: grnQty.INITIAL, recievedQty: grnQty.INITIAL, productRequest });

		await this.grnSubmitFooter.submitQuantity();

		await this.grnHeader.openCompletedGrn();

		await this.grnSidebar.validate({ recievedQty: requestDetails.qty, invoice: true, productRequest });

		productRequest.updateGrnStatus(GrnStatus.ORDER_RECEIVED);

		await this.grnInfoBox.validateGrnInfoBox({ productRequest });

		await this.grnProductAndServiceTable.validate({ deliveredQty: requestDetails.qty, recievedQty: requestDetails.qty, productRequest });
	}
	...
	async submitClientGrn({ productRequest }) {
		...
		await this.grnProductAndServiceTable.validate({ deliveredQty: requestDetails.qty, recievedQty: grnQty.INITIAL, productRequest });

		await this.grnHeader.openPendingGrn();

		await this.grnSidebar.submit({ recievedQty: grnQty.INITIAL, productRequest });

		await this.grnHeader.openCompletedGrn();

		await this.grnSidebar.validate({ recievedQty: requestDetails.qty, productRequest });
