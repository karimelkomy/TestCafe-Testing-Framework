...
export default class SubmitPaymentPage {
	...
	async submitPayment({ productRequest, bill }) {
		...
		const totalPaid =
			billDetails && billDetails.totalAdvanceAmount
				...
				: this.productsPrice.getTotalWithTaxAndShippingFeePrice({ requestDetails, rfqDetails, productsDetails: productsDetailsValue });
		...
		await this.vendorCredit.validateVendorCreditSection({ availableCredit, vendorDetails, organizationDetails });
	...
	async submitExpensePayment({ bill }) {
		...
		await this.selectBank.submitExpenseBankAccount({ totalPaid: TotalPaid.INITIAL, billDetails, organizationDetails });

		await this.paymentSubmitFooter.submit();

		await this.paymentSuccess.submit(); // BUG: Fix validation for new Post Payment Summary page.

		await this.paymentExpenseTable.validatePaymentTable({ billId, totalPaid, billDetails, workspaceDetails, organizationDetails });
