...
export default class ExpenseBillPage {
	...
	async createAndSubmitBillWithBudget({ bill }) {
		...
		const newBudgetDetails = await this.billExpenseAccount.calculateNewBudgetDetails({ bill });

		await this.billInfoBox.validateUploadedInvoice();

		await this.billExpenseAccount.validateExpenseAccountBalance({ billDetails, budgetDetails: newBudgetDetails, organizationDetails });
