...
export default class BuyerOrdersPage {
	...
	async submitClientOrderWithoutBudget({ productRequest, approveOrder, approverUserDetails }) {
		const {
			requestDetails,
			rfqDetails,
			workspaceDetails,
		...
		await this.ordersHeader.validateOrdersHeader({ requestDetails, requesterUserDetails: clientRequesterUserDetails });
		...
		await this.ordersPriceSummary.validateSummary({ productsDetails, organizationDetails, requestDetails, rfqDetails, markupDetails });
		...
		if (approveOrder) {
			...
			await this.orderApproval.approveOrder({ orderId, orderDetails, approverUserDetails });
	...
	async submitOrderWithoutBudget({ productRequest }) {
		...
		await this.ordersHeader.validateOrdersHeader({ requestDetails, requesterUserDetails, vendorDetails });

		await this.ordersInfoBox.validateOrdersInfoBox({ orderStatus, rfqDetails, workspaceDetails, organizationLocation, rfqId });

		await this.termsAndConditionsSidebar.selectTermsAndConditions({ orderDetails });
		...
		await this.ordersPriceSummary.validateSummary({ productsDetails, organizationDetails, requestDetails, rfqDetails });
	...
	async submitOrderWithBudget({ productRequest, approveOrder, approverUserDetails }) {
		const {
			requestDetails,
			rfqDetails,
			workspaceDetails,
		...
		await this.ordersExpenseAccount.selectExpenseAccounts({ productsDetails, budgetDetails, organizationDetails });

		await this.ordersPriceSummary.validateSummary({ productsDetails, organizationDetails, requestDetails, rfqDetails });

		await this.ordersExpenseAccount.validate({ productsDetails, budgetDetails, organizationDetails });
		...
		await this.ordersExpenseAccount.validate({ productsDetails, budgetDetails: newBudgetDetails, organizationDetails });
	}
	...
	async submitOrder({ productRequest, approveOrder, approverUserDetails }) {
		...
		if (budgetDetails) {
			await this.submitOrderWithBudget({ productRequest, approveOrder, approverUserDetails });
		} else {
