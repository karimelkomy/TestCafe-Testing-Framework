...
export default class CreateNewProductPage {
	...
	async createProduct({ organizationDetails, catalogueDetails, productsDetails, preferredVendorDetails }) {
		for (let productDetails of productsDetails) {
			await this.modulesSideBar.clickProductAndCataloguesLink();
			await this.productsBox.clickCreateProductButton();
			await this.productInfo.submit({ productDetails, catalogueDetails, preferredVendorDetails });
			await this.productDetails.submitProductDetails({ productDetails });
			await this.productMedia.submitProductMedia();
			await this.productPricing.submitProductPricing({ productDetails, organizationDetails });
			...
			await this.viewProductPage.validateProduct({ productDetails, organizationDetails });
		}
	}
}
