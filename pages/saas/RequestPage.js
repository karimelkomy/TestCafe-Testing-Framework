...
export default class RequestPage {
	...
	async createProductRequestWithoutSourcing({ productRequest }) {
		...
		await this.addProductsAndServices.submitCatalogProductRequest({ productRequest });
	...
	async createProductRequestWithRfqSourcingFromPreferredVendor({ productRequest }) {
		await this.requestInfo.submit({ productRequest });
		await this.addProductsAndServices.submitCatalogProductRequest({ productRequest });
	...
	async createManualProductRequestWithRfqSourcing({ productRequest }) {
		await this.requestInfo.submit({ productRequest });
		await this.addProductsAndServices.submitManualProductRequest({ productRequest });
		await this.sourcing.submitRfqFromVendors({ productRequest });
	}
	...
	async approveProductRequest({ productRequest }) {
		...
		await this.requestApproval.approveRequest({ requestId, requestDetails, requesterUserDetails });
	}

	async approveClientProductRequest({ productRequest }) {
		const { requestId, requestDetails, clientRequesterUserDetails } = productRequest;

		await this.requestApproval.approveRequest({ requestId, requestDetails, requesterUserDetails: clientRequesterUserDetails });
	}
	...
	async createProductRequest({ productRequest }) {
		...
		if (requestSourcing === RequestSourcing.SOURCE_FROM_VENDORS) {
			await this.createProductRequestWithRfqSourcingFromVendors({ productRequest });
		} else if (requestSourcing === RequestSourcing.MANUAL_WITH_SOURCE_FROM_VENDORS) {
			await this.createManualProductRequestWithRfqSourcing({ productRequest });
		} else if (requestSourcing === RequestSourcing.BULK_WITH_SOURCE_FROM_VENDORS) {
			await this.createBulkProductRequestWithRfqSourcing({ productRequest });
		} else if (requestSourcing === RequestSourcing.ORDER_WITH_SOURCE_FROM_VENDORS) {
			await this.createProductRequestWithOrderSourcing({ productRequest });
		} else if (requestSourcing === RequestSourcing.ORDER_WITH_SOURCE_FROM_CONTRACT) {
			await this.createProductRequestWithOrderSourcingUsingContract({ productRequest });
		} else if (requestSourcing === RequestSourcing.SOURCE_FROM_PREFERRED_VENDOR) {
			await this.createProductRequestWithRfqSourcingFromPreferredVendor({ productRequest });
		} else if (requestSourcing === RequestSourcing.SOURCE_FROM_CONTRACT) {
			await this.createProductRequestWithRfqSourcingUsingContract({ productRequest });
