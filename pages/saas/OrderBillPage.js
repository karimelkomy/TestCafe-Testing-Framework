...
export default class OrderBillPage {
	...
	async validatePaidBillBox({ productRequest }) {
		const { requestDetails, rfqDetails, workspaceDetails, organizationDetails, productDetails, productsDetails, billUrl } = productRequest;
	...
	async submitOrderBill({ productRequest, approveBill, approverUserDetails }) {
		...
		await this.billPriceSummary.validateSummary({ organizationDetails, requestDetails, rfqDetails, productsDetails: productsDetailsValue });
		...
		if (approveBill) {
			await this.billApproval.approveBill({ billId, billDetails, approverUserDetails });
			await this.billSuccess.submit();
		}
	...
	async submitClientOrderBill({ productRequest, approveBill, approverUserDetails }) {
		const {
			requestDetails,
			rfqDetails,
			productDetails,
		...
		if (approveBill) {
			await this.billApproval.approveBill({ billId: clientBillId, billDetails, approverUserDetails });
			await this.billSuccess.submit();
		}
