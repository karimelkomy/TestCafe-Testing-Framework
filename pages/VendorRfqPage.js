...
export default class VendorRfqPage {
  ...
  async validateSubmittedRfq({
    ...
    await this.rfqSummary.validateSummary({
      organizationDetails,
      requestDetails,
      rfqDetails,
      productsDetails,
  ...
  async navigateToVendorRfqUrl(rfqUrl, vendorDetails) {
    const requestUuid = await rfqUrl.substring(
      rfqUrl.indexOf("request/") + "request/".length,
      rfqUrl.indexOf("/detail")
    );
    const vendorRfqUrl = await VendorRfqApi.getVendorRfqUrl(
      requestUuid,
      vendorDetails
    );
  ...
  async openRfq({ productRequest }) {
    ...
    if (email === "true") {
      await this.vendorAppAccess.openRfq({
        rfqId,
        vendorDetails,
        text: rfqDetails.rfqType
      });
    } else {
  ...
  async submitRfq({ productRequest }) {
    ...
    await this.rfqInfoBox.submitInfoBox({
      paymentTermMode,
      rfqDetails,
      vendorDetails
    });
    ...
    await this.rfqSubmission.submitOffer({
      requestDetails,
      rfqDetails,
      organizationDetails,
      productsDetails
  ...
  async acceptNegotiatedOffer({ productRequest }) {
    ...
    await this.rfqProductAndServiceTable.submitNegotiation({
      organizationDetails,
      rfqDetails,
      productsDetails
    });

    await this.rfqInfoBox.submitNegotiation({
      rfqDetails,
      organizationDetails
    });
