...
export default class ReimbursementBillPage {
  ...
  async validateReimbursementBill({
    ...
    await this.billExpenseTable.validateExpenseTable({
      reimbursementDetails,
      organizationDetails
    });
  ...
  async submitReimbursementBillWithoutBudget({ reimbursementRequest }) {
    ...
    await this.validateReimbursementBill({
      reimbursementDetails,
      workspaceDetails,
      organizationDetails,
      requesterUserDetails
  ...
  async submitReimbursementBillWithBudget({ reimbursementRequest }) {
    ...
    await this.billExpenseAccount.submit({
      reimbursementDetails,
      budgetDetails,
      organizationDetails
    });
    ...
    const newBudgetDetails = await this.billExpenseAccount.calculateNewBudgetDetails(
      { budgetDetails, reimbursementDetails }
    );
