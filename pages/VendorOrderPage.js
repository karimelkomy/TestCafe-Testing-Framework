...
export default class VendorOrderPage {
  ...
  async navigateToVendorOrderUrl({ orderUrl, identifier }) {
    const orderUuid = orderUrl.substring(
      orderUrl.indexOf(`${identifier}/`) + `${identifier}/`.length
    );

    const vendorOrderUrl = await VendorOrderApi.getVendorOrderUrl(orderUuid);

    await this.vendorAppAccess.navigateToVendorUrl({
      vendorUrl: vendorOrderUrl
    });
  }
  ...
  async openOrder({ productRequest }) {
    ...
    if (email === "true") {
      await this.vendorAppAccess.openOrder({
        orderId,
        vendorDetails,
        text: "Order"
      });
  ...
  async acceptOrder({ productRequest }) {
    ...
    await this.orderProductAndServiceTable.validateProductAndServiceTable({
      productRequest
    });
    ...
    await this.orderTermsAndConditions.validateTermsAndConditions({
      productRequest
    });
